...
class BathymetricAnalyzer:
    ...
    def _calculate_sinuosity(self, thalweg: np.ndarray) -> float:
        if thalweg is None or len(thalweg) < 2:
            return 1.0
        path_length = float(np.sum(np.sqrt(np.sum(np.diff(thalweg, axis=0) ** 2, axis=1))))
        straight_distance = float(np.sqrt(np.sum((thalweg[-1] - thalweg[0]) ** 2)))
