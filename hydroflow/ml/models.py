...
class SedimentationPredictor:
    ...
    def prepare_features(self, flow_data: pd.DataFrame, precipitation: pd.DataFrame, land_use: pd.DataFrame, bathymetry: np.ndarray) -> np.ndarray:
        features: List[float] = []
        features.extend([
    ...
    def train(self, X: np.ndarray, y: np.ndarray, validation_split: float = 0.2) -> Dict:
        ...
        train_score = self.model.score(X_train_scaled, y_train) if hasattr(self.model, "score") else 0.0
    ...
    def predict(self, X: np.ndarray, return_uncertainty: bool = False) -> Union[np.ndarray, Tuple[np.ndarray, np.ndarray]]:
        ...
        if self.model_type == "ensemble" and return_uncertainty and hasattr(self.model, "estimators_"):
...
class FlowPatternPredictor:
    ...
    def __init__(self, input_size: int, hidden_sizes: List[int], output_size: int):
        ...
        self.weights = [rng.standard_normal((sizes[i], sizes[i + 1])) * 0.1 for i in range(len(sizes) - 1)]
